...
for two things this project already half-has informally: the validators
library, and a not-yet-built reference library for Data Cloud/industry-
specific migration patterns. Genuinely new — the repo was created
2026-05-04, after Claude's training cutoff, and was still being actively
pushed to as of 2026-07-10 — so this entry is grounded directly in the
real `SPEC.md` text (fetched and read via `gh api`), not a guess from
stale training data.

**What OKF actually is**: deliberately minimal — a directory of markdown
files ("a Knowledge Bundle"), one file per "Concept," designed for both
human and agent consumption with no required tooling, schema registry, or
central authority. The rules that matter here:

- A Concept's **Concept ID** is its file path minus `.md` (`tables/
  users.md` → `tables/users`).
- Every concept needs YAML frontmatter with exactly one **required**
  field: `type` (a free-text string, e.g. `BigQuery Table`, `Playbook`,
  `Reference` — no central registry; consumers MUST tolerate unknown
  types). **Recommended** fields, in priority order: `title`,
  `description`, `resource` (a URI to the real underlying asset — omitted
  for a concept describing an abstract idea rather than a physical
  resource), `tags`, `timestamp`.
- Two **reserved filenames**, no frontmatter: `index.md` (a directory
  listing for progressive disclosure) and `log.md` (a chronological,
  date-headed change log — `## YYYY-MM-DD` sections, newest first).
- Cross-links are plain markdown links, bundle-relative (`/tables/
  customers.md`) or relative — untyped, meaning comes from the
  surrounding prose, and **consumers MUST tolerate broken links**
  gracefully (§9's own conformance rule: an unresolved link is
  "not-yet-written knowledge," not a malformed document).
- A `# Citations` section (numbered, external URLs or bundle-relative
  paths) is the conventional place for claims sourced from outside
  material.
- **Conformance is deliberately loose** (§9): a bundle is conformant if
  every non-reserved `.md` has parseable frontmatter with a non-empty
  `type` — everything else (missing optional fields, unknown types,
  unknown extra keys, broken links, missing `index.md`) is soft guidance,
  never a rejection reason. This permissiveness is explicitly by design,
  "meant to remain useful as bundles grow, get refactored, and are
  partially generated by agents."
- Explicitly a **non-goal**: replacing domain-specific schemas (Avro,
  Protobuf, OpenAPI, etc.) — OKF *references* those via `resource:`, it
  doesn't subsume them.

**Why this is a genuine fit, not a stretch**:

- `validators/system/*.md` and `validators/<Object>.md` are already
  exactly one-Concept-per-file, and already grow "via real corrections"
  the same way an OKF bundle is meant to grow — they're just missing the
  frontmatter. Adding `type: SystemValidator` / `type: ObjectValidator`,
  a `resource:` pointing at the real CLI command or Salesforce object
  describe() URL, and `tags:` would make them parseable by any
  OKF-aware tool, not just this repo's own bespoke `validators_lookup.py`
  — while changing nothing about how a human reads them today, since the
  body stays plain markdown either way.
- The "how a mapping doc / run book gets built" knowledge currently lives
  embedded in `CLAUDE.md`'s own canonical-commands prose. Breaking
  pieces of that into standalone `type: Workflow` concept files with
  `resource:` links to the real implementing module (`mapping_doc.py`,
  `migration_run_book.py`) would make them independently retrievable
  instead of requiring the whole file to be loaded for one workflow step.
- The dual human+agent consumption model is exactly this project's own
  operating convention already — `CLAUDE.md` is read by both a human
  architect and every Claude Code session that opens this repo, same as
  an OKF bundle is meant to be read by both a person and a consumption
  agent.

**Where I'd say no, don't force it**: `sql/transformations/*.sql` scripts
should stay real `.sql` files, never get wrapped into OKF markdown —
doing so would break the Script Ticket Traceability Rule (#10), plain
git-diffability, and the "everything reviewable as real SQL" trust model
this whole framework is built on. The right-sized fit matches OKF's own
`resource:` pattern instead: a concept file describing the transform
*pattern* (`type: Pattern`, prose explaining the shape) with `resource:`
linking to a real example script — index and describe the SQL, don't
contain it.

**The Data Cloud/industry-pattern idea — where `resource:` earns its
keep**: a concept file doesn't need to duplicate Salesforce Help content
(which would go stale and violate this project's own "don't guess from
training data, check live docs" discipline). It can be a thin, curated
file (`type: D360Concept` or `type: IndustryPattern`, a few sentences of
this project's own migration-specific notes) with `resource:` pointing at
the authoritative `developer.salesforce.com`/`help.salesforce.com` page —
a real, low-maintenance way to build a growable reference library without
this framework becoming responsible for keeping Salesforce's own
documentation in sync.

**Proposed plan (roadmap idea only — nothing built yet)**:

1. Pilot on `validators/` first — smallest, cleanest gap between current
   state and OKF-conformant, and it's the piece already closest to
   "grows via real corrections." Add frontmatter to existing files;
   change no body content.
2. A new top-level `okf/` (or similar) directory for the industries/
   data-model/Data Cloud reference bundle, seeded with a handful of real,
   tagged concept files rather than trying to cover everything up front
   — same "ship the simple version first" discipline as
   `generate-run-book-flowchart`/`generate-pass-summary`'s own v1 framing.
3. Leave `sql/transformations/*.sql` exactly as-is; any OKF concept files
   in this space describe and link to the real scripts, never replace
   them.
4. Decide later (not now) whether `validators_lookup.py` should become a
   thin OKF-conformant reader, or stay bespoke — a real design question
   once the pilot shows whether the frontmatter genuinely earns its keep
...
## 79. `analyze-org-risk` learns to see auto-generated child records (BUILT 2026-07-18)

Direct follow-up to roadmap #77's `GiftCommitmentSchedule` finding
(itself found via #78's `sample-reference-records`). The user's own
diagnosis of that bug asked the sharper question: why didn't
`analyze-org-risk` catch this before the load? Answer, confirmed by
checking live: it structurally can't. `risk_analyzer.py` only sees
automation the Tooling API can introspect — `ValidationRule`/
`ApexTrigger`/record-triggered `Flow`/`WorkflowRule`/`ApprovalProcess`.
Nonprofit Cloud auto-creating a `GiftCommitmentSchedule` when a
`GiftCommitment.ScheduleType = 'Recurring'` is inserted is
managed-package-internal logic — no Flow/trigger metadata exists for the
Tooling API to find, confirmed by directly checking `GiftCommitment` and
finding nothing.

**The fix, per the user's own proposed approach**: since the automation
itself can't be introspected, infer it empirically instead — diff real,
human-created/pre-existing parent records in the target org against
their real children. New `child_record_risk.py`,
`detect_auto_generated_children(sf, object_names, edges=None, ...)`:
reuses `load_order.build_dependency_edges()` (no new org-wide reverse-
lookup infra needed, since `analyze-org-risk` is always invoked with the
full object set a migration project is analyzing anyway) to find every
parent→child dependency edge among the given objects, samples real
non-migrated parent Ids (`MigrationID__c = null`, the same real-reference-
data signal #78's `sample-reference-records` already established — falls
back to an unfiltered sample if the object has no `MigrationID__c` field,
flagged as such), and runs one aggregate `GROUP BY` query per edge (not
one query per sampled record) to compute what fraction of real parents
already have a real child. A consistently high rate (default threshold
80%, minimum sample 3) is reported as `likely_auto_generated` — advisory,
not proof; a normal 1:1 business relationship can show the same pattern,
so the finding always says "verify before assuming," never "this is
definitely platform automation."

**Integration, no new table**: `risk_analyzer.analyze_migration_risk()`
gained `check_child_shape=True` (the default — this is the main gap
`analyze-org-risk` had, not an opt-in extra step to remember), and
`write_to_sql()` writes a 6th `CheckType = "AutoGeneratedChildRecord"`
into the existing `dbo.ObjectAutomationRisk` table — reusing its
free-text `CheckType` column rather than a parallel table, so every
existing consumer (`sample-reference-records`'s own automation summary,
`orchestrator.py`'s cold-start check) picks it up with zero changes.
...
`GiftCommitment -> GiftCommitmentSchedule` — it caught a different, also
genuine, auto-generation pattern instead
(`GiftCommitmentSchedule -> GiftTransaction`, 100% of 10 real schedules
already have a transaction — Nonprofit Cloud's own scheduled transaction-
creation automation, `Type = 'CreateTransactions'`). Investigating why the
expected edge was missed found the real cause: this org's broader
non-migrated `GiftCommitment` population mixes `Recurring`- and
`Custom`-type records together, diluting the true rate — a real, direct
query showed only 6 of 10 sampled real `GiftCommitment` records have a
real `GiftCommitmentSchedule` (60%), below the original 0.8 default
threshold. `DEFAULT_THRESHOLD` was recalibrated to 0.5 based on this real
evidence (not a guess) — re-running the same live command with the fix
now correctly flags `GiftCommitment -> GiftCommitmentSchedule` at 60%,
confirmed directly. A more precise future version could segment the
sample by whatever field predicts the automation (`ScheduleType` here)
rather than treating the whole object population as one group — not
built now, noted as a real known limitation instead of oversold.

**Tests**: `tests/test_child_record_risk.py` (new) — high-rate detection,
low-rate (not flagged), insufficient sample size (never flagged
regardless of rate), missing-`MigrationID__c` fallback, the aggregate
query's exact shape, zero-edges, and the `edges=None` fallback to
`build_dependency_edges`. `tests/test_risk_analyzer.py` — new coverage
for the `AutoGeneratedChildRecord` row shape and the below-threshold
case still getting the `ScanCompleted` marker. Full suite green
throughout.
...
(`"PersonAccount_Load]\n    ) a\n),\nCampaignPool AS (\n    SELECT Id"`)
as if they were target field names, while nearly every genuinely
implemented column (`Name`, `Status`, `ScheduleType`, etc.) was reported
as "documented as mapped, but the transform doesn't populate."

**Root cause, confirmed by direct reproduction**:
`mapping_doc.py`'s `_SELECT_INTO_RE` (`SELECT\s+(?P<cols>.*?)\s+INTO\s+...`,
non-greedy but `re.DOTALL`) is matched via `.finditer()` against the
*whole* script text with no awareness of CTEs. A script shaped like
`WITH cte1 AS (SELECT ...), cte2 AS (SELECT ...) SELECT <real cols>
INTO <table> FROM ...` has no `INTO` keyword anywhere inside the CTE
bodies, so the regex engine's non-greedy `.*?` — starting from the
very *first* `SELECT` in the text, inside the first CTE — has nothing
to stop it short of the real `INTO` much later, and ends up capturing
every CTE's own internal SELECT list (window-function aliases, nested
sub-selects, even the comma/parenthesis punctuation between CTEs) as
the "implemented column list." This affected the majority of this
build's own newer scripts (`300`, `310`, `320`, `360`, `390`, `420`,
`430` all use a leading `WITH` clause) — a real, previously-undetected
gap, since `check-mapping-balance` had never been run against a
CTE-shaped script before this pass.

**Fix**: a new `_strip_leading_ctes()` helper walks a well-formed,
comma-separated `WITH cte AS (...), cte2 AS (...) <final statement>`
preamble via balanced-parenthesis tracking (not a full SQL parser —
falls back to returning the text unchanged if the shape doesn't match
what's expected, the safe conservative direction), and
`extract_insert_columns()` now applies it before running the
`SELECT...INTO`/`CREATE TABLE AS SELECT` regexes. Handles the CTE
appearing anywhere a real SQL statement can start in this project's own
scripts — not just at the literal start of the file, since every real
script here begins with a `DROP TABLE IF EXISTS ...;` statement first
(found live: an earlier version of the fix anchored to the absolute
start of the string and silently no-op'd on every real script, since
none of them actually start with `WITH` literally at position 0).

Verified directly against 5 of this build's own real CTE-based scripts
(`300`, `270`, `360`, `390`, `420`, `430`) — each now returns exactly
the real, correct implemented-column list. Live-rerun of
`check-mapping-balance GiftCommitment ...` against the real mapping
workbook now reports only genuine, expected drift (bookkeeping columns
