...
A real, confirmed blind spot in all of the above (found 2026-07-18, see
child_record_risk.py and okf/nonprofit-cloud/gift-commitment-schedule-auto-creation.md):
managed-package-internal automation -- e.g. Nonprofit Cloud silently
auto-creating a GiftCommitmentSchedule when a GiftCommitment is inserted
-- has no visible Flow/trigger/rule the Tooling API can surface.
analyze_migration_risk() also runs child_record_risk.py's empirical
diff-against-real-data check by default (check_child_shape=True) to catch
this class of issue, since it can't be found by metadata introspection
alone.
"""
import os
import re
...
import sql_dialect
from child_record_risk import detect_auto_generated_children
...
def analyze_migration_risk(sf, object_names, fields_in_scope_by_object=None,
                            check_child_shape=True):
    """check_child_shape=True (default) also runs
    child_record_risk.detect_auto_generated_children() over every
    dependency edge among object_names -- catches automation the Tooling
    API can't see (managed-package-internal auto-creation of a child
    record), by empirically diffing real target-org data instead of
    introspecting metadata. See child_record_risk.py's own module
    docstring for the full rationale. One extra live query per real
    dependency edge in object_names -- pass check_child_shape=False to
    skip for a large/slow object list."""
    ...
    if check_child_shape:
        edges = build_dependency_edges(sf, object_names)
        child_findings = detect_auto_generated_children(sf, object_names, edges=edges)
        findings_by_parent = {}
        for f in child_findings:
            findings_by_parent.setdefault(f["parent"], []).append(f)
        for result in results:
            result["auto_generated_child_findings"] = findings_by_parent.get(result["object"], [])
    else:
        for result in results:
...
def write_to_sql(engine, results, schema="dbo"):
    ...
    for result in results:
        ...
        # .get(), not [] -- a result dict built by hand (e.g. an older test
        # fixture, or analyze_migration_risk(check_child_shape=False)'s own
        # empty-list default) may not carry this key at all.
        for f in result.get("auto_generated_child_findings", []):
            if not f["likely_auto_generated"]:
                continue
            rows.append({
                "object_name": obj, "check_type": "AutoGeneratedChildRecord",
                "item_name": f["child"], "is_active": True, "direct_hit": False,
                "detail": (
                    f"{f['with_child_count']}/{f['sample_size']} real {obj} records "
                    f"already have a real {f['child']} via {f['field']} ({f['rate']:.0%})"
                ),
                "analyzed_at": analyzed_at,
            })
