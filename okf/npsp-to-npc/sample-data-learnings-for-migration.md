...
| --- | --- | --- | --- |
| `AccountContactRelation` — **two distinct records** | see next cell | `110_npc_accountcontactrelation_load` | **Corrected 2026-07-27 during the v2 build** (the sample data misled the first draft here). The `IsDirect=true` **self**-relationship (Person Account ↔ its own shadow Contact) is platform-auto-created and left alone (never insert *or* update — `sql/transformations/250` is doc-only; real rows are `IsIncludedInGroup=False`/`IsPrimaryMember=False`). But the PoC's `110` inserts the *other* record — the `IsDirect=false` **household-membership** ACR (household Account → each member's shadow Contact, `IsIncludedInGroup=true`), which is **not** auto-created and **is** the confirmed household-grouping mechanism (2026-07-18 architect review). Keep that insert. The sample data had no multi-member households, so its "doc-only" conclusion didn't cover this case. See [account-contact-relation-auto-creation](../nonprofit-cloud/account-contact-relation-auto-creation.md), `validators/AccountContactRelation.md`, and this attempt's `030` header. |
