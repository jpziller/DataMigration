...
Illustrative shape (steps 1–4; a hypothetical source table, not a real
script — this is style guidance to read, not something to copy into
`sql/transformations/` and run. A real numbered script for an actual
object/project is always built fresh against that project's own real
source table and real mapping decisions):
...
This example used to live as a real, numbered file directly inside
`sql/transformations/` (`010_account_load.sql`). It was moved here
(2026-07-11) because that folder is reserved for real, project-specific
scripts only — see CLAUDE.md's "Where things live" — and a style example
sitting among real numbered scripts risked being mistaken for one (or
swept up in a "wipe every numbered script" reset alongside genuine test
artifacts, which is exactly what happened). Style/pattern knowledge like
this belongs in documentation, not in the numbered script sequence.
